...
2.  **`validateUFO.py`:**
    ...
    *   **Functionality:** Reads a directory of predicted inundation masks (e.g., the mosaicked predictions from `singlePipeline_timmModels.py`) and a corresponding directory of ground truth labels. It calculates per-file confusion matrices and standard segmentation metrics (Precision, Sensitivity, Specificity, F1, IoU, Accuracy). This script can be used to evaluate prediction sets generated by `oneSinglePipeline.py` or other methods, saving the results to a CSV file.
    *   **Usage:**
